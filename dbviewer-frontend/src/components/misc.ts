...
export function Badge(text: Child, tone: BadgeTone = "neutral", opts: { icon?: string } = {}): HTMLElement {
  return el("span.gtma-badge", { class: `gtma-badge--${tone}` },
...
export function Spinner(size = 16): HTMLElement {
  return el("span.gtma-spinner", { style: { width: `${size}px`, height: `${size}px` }, attrs: { role: "status", "aria-label": "Loading" } });
}
...
export function EmptyState(opts: { icon?: string; title: string; description?: string; action?: HTMLElement }): HTMLElement {
  return el("div.gtma-empty", {},
    opts.icon ? el("div.gtma-empty__icon", {}, icon(opts.icon)) : null,
    el("div.gtma-empty__title", {}, opts.title),
    opts.description ? el("div.gtma-empty__desc", {}, opts.description) : null,
    opts.action ?? null,
  );
}
...
export function LoadingState(label = "Loading…"): HTMLElement {
  return el("div.gtma-loading", {}, Spinner(20), el("span.muted", {}, label));
}
...
export function Segmented(opts: { options: SegmentedOption[]; value?: string; onChange?: (v: string) => void }): HTMLElement {
  ...
  const root = el("div.gtma-segmented", { attrs: { role: "tablist" } },
    ...opts.options.map((o) => {
      const btn = el("button.gtma-segmented__btn", {
...
export function Field(label: string, value: Child): HTMLElement {
  return el("div.gtma-field", {},
    el("div.gtma-field__label", {}, label),
    el("div.gtma-field__value", {}, value),
  );
}
...
export function Switch(opts: { checked?: boolean; onChange?: (checked: boolean) => void; label?: string }): HTMLElement {
  ...
  const knob = el("span.gtma-switch__track", { class: checked ? "is-on" : "" }, el("span.gtma-switch__knob"));
  const root = el("button.gtma-switch", {
  ...
  }, knob, opts.label ? el("span.gtma-switch__label", {}, opts.label) : null);
  return root;
}
