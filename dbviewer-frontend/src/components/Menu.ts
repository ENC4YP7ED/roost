...
export function buildMenuPanel(items: MenuItem[], close: () => void): HTMLElement {
  const panel = el("div.gtma-menu", { attrs: { role: "menu" } });

  for (const item of items) {
    if (item.separator) {
      panel.appendChild(el("div.gtma-menu__sep"));
      continue;
    }
    if (item.header) {
      panel.appendChild(el("div.gtma-menu__header", {}, item.header));
      continue;
    }

    const row = el("button.gtma-menu__item", {
      class: [item.danger ? "gtma-menu__item--danger" : "", item.disabled ? "gtma-menu__item--disabled" : ""].join(" "),
      ...
      el("span.gtma-menu__icon", {}, item.checked ? icon("check") : item.icon ? icon(item.icon) : null),
      el("span.gtma-menu__label", {}, item.label ?? ""),
      item.shortcut ? el("span.gtma-menu__shortcut.mono", {}, item.shortcut) : null,
      item.submenu ? el("span.gtma-menu__chevron", {}, icon("chevron-right")) : null,
    );
