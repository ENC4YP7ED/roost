...
export function Scroller(content: HTMLElement, opts: { class?: string } = {}): ScrollerHandle {
  const viewport = el("div.gtma-scroller__viewport", {}, content);
  const vThumb = el("div.gtma-scroller__thumb.gtma-scroller__thumb--v");
  const hThumb = el("div.gtma-scroller__thumb.gtma-scroller__thumb--h");
  const vTrack = el("div.gtma-scroller__track.gtma-scroller__track--v", {}, vThumb);
  const hTrack = el("div.gtma-scroller__track.gtma-scroller__track--h", {}, hThumb);

  const root = el("div.gtma-scroller", { class: opts.class ?? "" }, viewport, vTrack, hTrack);
