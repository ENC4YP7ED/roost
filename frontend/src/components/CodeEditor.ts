...
export function CodeEditor(opts: CodeEditorOptions): CodeEditorHandle {
  const gutter = el("div.rst-editor__gutter", { attrs: { "aria-hidden": "true" } });
  const highlight = el("pre.rst-editor__highlight", { attrs: { "aria-hidden": "true" } });
  const textarea = el("textarea.rst-editor__textarea", {
    attrs: {
  ...
  const root = el("div.rst-editor", {}, gutter, el("div.rst-editor__surface", {}, highlight, textarea));
  ...
  function paint() {
    ...
    if (gutter.childElementCount !== want) {
      ...
      for (let i = 1; i <= want; i++) gutter.appendChild(el("div.rst-editor__lineno", {}, String(i)));
    }
  }
