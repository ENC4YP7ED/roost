...
export function Tree(): TreeHandle {
  const list = el("div.rst-tree", { attrs: { role: "tree" } });
  ...
  function renderNode(node: TreeNode): HTMLElement {
    ...
    const wrapper = el("div.rst-tree__node");
    const childRows = el("div.rst-tree__children");
    let loaded = false;
    let loading = false;

    const chevron = expandable
      ? icon("chevron-right", { class: "rst-tree__chevron" })
      : el("span.rst-tree__chevron-spacer");

    const row = el("div.rst-tree__row", {
      ...
      chevron,
      icon(node.icon ?? "circle", { class: "rst-tree__icon" }),
      el("span.rst-tree__label.truncate", {}, node.label),
      node.badge != null ? el("span.rst-tree__badge", {}, String(node.badge)) : null,
    );
    ...
    async function toggle() {
      ...
      if (node.iconOpen) {
        const ic = row.querySelector(".rst-tree__icon");
        ic?.replaceWith(icon(node.iconOpen, { class: "rst-tree__icon" }));
      }
      if (!loaded && !loading) {
        loading = true;
        ...
        try {
          ...
          clear(childRows);
          if (!kids.length) {
            childRows.appendChild(el("div.rst-tree__empty", { style: { paddingLeft: `${(node.level + 1) * 14 + 8}px` } }, "empty"));
          }
          for (const k of kids) childRows.appendChild(renderNode(k));
          loaded = true;
        } catch (err) {
          clear(childRows);
          childRows.appendChild(el("div.rst-tree__error", { style: { paddingLeft: `${(node.level + 1) * 14 + 8}px` } }, String(err)));
        } finally {
          loading = false;
        }
      }
  ...
  function childRowsSetLoading(childRows: HTMLElement) {
    clear(childRows);
    childRows.appendChild(el("div.rst-tree__loading", {}, Spinner(13), el("span.faint", {}, "loading…")));
  }
  ...
  return {
    ...
    collapseAll: () => {
      expanded.clear();
      list.querySelectorAll(".rst-tree__node.is-open").forEach((n) => n.classList.remove("is-open"));
    },
  };
}
