...
export function DataGrid(opts: DataGridOptions): DataGridHandle {
  ...
  const root = el("div.rst-grid__scroll", {}, table) as HTMLDivElement;
  ...
  function render(columns: GridColumn[], rows: Array<Array<string | null>>, sortBy?: string, sortDir?: string) {
    ...
    if (!rows.length) {
      table.appendChild(el("tbody", {}, el("tr", {},
        el("td.rst-grid__empty", { attrs: { colspan: columns.length + 1 } }, "No rows"))));
      return;
    }
    ...
    const draw = () => {
      ...
      if (!calibrated) {
        ...
        requestAnimationFrame(() => {
          const r = tbody.querySelector("tr.rst-grid__row") as HTMLElement | null;
          if (r) {
  ...
  function buildHeader(columns: GridColumn[], sortBy?: string, sortDir?: string): HTMLElement {
    return el("thead", {}, el("tr", {},
      el("th.rst-grid__rownum", {}, "#"),
      ...columns.map((c) => {
        const sortable = !!opts.onSort;
        const isSorted = sortBy === c.name;
        return el("th.rst-grid__th", {
          class: [sortable ? "is-sortable" : "", isSorted ? "is-sorted" : ""].join(" "),
          onclick: sortable ? () => opts.onSort!(c.name) : undefined,
        },
          el("div.rst-grid__th-inner", {},
            c.primary ? icon("key", { class: "rst-grid__pk" }) : null,
            el("span.rst-grid__col-name", {}, c.name),
            c.type ? el("span.rst-grid__col-type.mono", {}, baseType(c.type)) : null,
            isSorted ? icon(sortDir === "desc" ? "arrow-down-long" : "arrow-up-long", { class: "rst-grid__sort" }) : null,
          ),
        );
      }),
    ));
  ...
  function buildBodyRow(ri: number, row: Array<string | null>, columns: GridColumn[]): HTMLElement {
    const tr = el("tr.rst-grid__row", {},
      el("td.rst-grid__rownum", {}, String((opts.rowNumberStart ?? 1) + ri)),
  ...
  function renderCell(cell: string | null, col: GridColumn): HTMLElement {
    if (cell === null) {
      return el("td.rst-grid__cell.rst-grid__cell--null", { attrs: { title: "NULL" } }, "NULL");
    }
    const numeric = col?.type ? NUMERIC.test(col.type) : false;
    const truncated = cell.length > 220 ? cell.slice(0, 220) + "…" : cell;
    if (col?.link) {
      return el("td.rst-grid__cell", { class: numeric ? "rst-grid__cell--num" : "" },
        el("button.rst-grid__fk", {
    ...
    return el("td.rst-grid__cell", {
      class: numeric ? "rst-grid__cell--num mono" : "",
...
function spacerRow(px: number, cols: number): HTMLElement {
  return el("tr.rst-grid__spacer", { attrs: { "aria-hidden": "true" } },
