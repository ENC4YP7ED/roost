...
export function Tabs(tabs: TabDef[], opts: { active?: string; onSelect?: (id: string) => void } = {}): TabsHandle {
  ...
  const tabList = el("div.rst-tabs__list", { attrs: { role: "tablist" } });
  const panel = el("div.rst-tabs__panel.grow");
  ...
  for (const tab of tabs) {
    const btn = el("button.rst-tabs__tab", {
      attrs: { type: "button", role: "tab" },
      onclick: () => select(tab.id),
    },
      tab.icon ? icon(tab.icon) : null,
      el("span", {}, tab.label),
      tab.badge != null ? el("span.rst-tabs__badge", {}, String(tab.badge)) : null,
    ) as HTMLButtonElement;
    buttons.set(tab.id, btn);
    tabList.appendChild(btn);
  }

  const root = el("div.rst-tabs.col.grow", {}, tabList, panel);
